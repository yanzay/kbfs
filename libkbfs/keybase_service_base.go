...
type KeybaseServiceBase struct {
	...
	lastSyncNotificationPathname string
}
...
func (k *KeybaseServiceBase) NotifySyncStatus(ctx context.Context,
	...
	func() {
		k.lastNotificationFilenameLock.Lock()
		defer k.lastNotificationFilenameLock.Unlock()
		if status.Pathname != k.lastSyncNotificationPathname {
			k.lastSyncNotificationPathname = status.Pathname
			k.log.CDebugf(ctx, "Sending notification for %s", status.Pathname)
		}
	}()
